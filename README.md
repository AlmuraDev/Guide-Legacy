...
__For [Eclipse]__  
  ...
  3. Import Guide as a Gradle project (File > Import)
  4. Select the root folder for Guide and click **Build Model**
  5. Check Guide when it finishes building and click **Finish**

__For [IntelliJ]__  
  1. Run `gradle setupDecompWorkspace --refresh-dependencies`  
  2. Make sure you have the Gradle plugin enabled (File > Settings > Plugins).  
  3. Click File > Import Module and select the **build.gradle** file for Guide.

## Running
__Note 1:__ The following is aimed to help you setup run configurations for Eclipse and IntelliJ, if you do not want to be able to run Guide directly from your IDE then you can skip this.

__For [Eclipse]__  
  1. Go to **Run > Run Configurations**.  
  2. Right-click **Java Application** and select **New**.  
  3. Set the current project.  
  4. Set the name as `Guide (Client)` and apply the information for Client below.
  5. Repeat step 1 through 4, then set the name as `Guide (Server)` and apply the information for Server below.  
...
__For [IntelliJ]__  
  ...
  3. Set the name as `Guide (Client)` and apply the information for Client below.  
  4. Repeat step 2 and set the name as `Guide (Server)` and apply the information for Server below.  
...
| Module classpath  | Guide (IntelliJ Only)                                     |
...
In order to build Guide you simply need to run the `gradle` command. You can find the compiled JAR files in `./build/libs` but in most cases you'll only need 'guide-x.x-xxxx-x.x.x.jar'.
